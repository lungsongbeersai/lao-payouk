<?php session_start();
    ...
    if(isset($_GET["fetch_data"])){
        ...
        @$query.="res_zone AS A LEFT JOIN res_branch AS B ON A.zone_branch_fk=B.branch_code";
...
?>
            <tr class="table_hover" ondblclick="edit_function('<?php echo $row_sql['zone_code'];?>','<?php echo $row_sql['zone_branch_fk'];?>',,'<?php echo $row_sql['zone_name'];?>')">
                <td align="center"><?php echo $total_id++;?></td>
                <td><?php echo $row_sql["zone_name"]?></td>
                <td><?php echo $row_sql["branch_name"]?></td>
                <td align="center">
                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="edit_function('<?php echo $row_sql['zone_code'];?>','<?php echo $row_sql['zone_branch_fk'];?>','<?php echo $row_sql['zone_name'];?>')">
...
<?php }
    if(isset($_GET["insert_data"])){
        ...
        if($_POST["zone_code"] !=""){
            $sql="zone_branch_fk='".$_POST["zone_branch_fk"]."',zone_name='".$_POST["zone_name"]."' WHERE zone_code='".$_POST["zone_code"]."'";
            ...
            $sql="'".$auto_number."','".$_POST["zone_branch_fk"]."','".$_POST["zone_name"]."'";

...
if(isset($_GET["loadOrders"])){
    // if($_POST["idStatus"]=="4"){
    //     $status="AND order_list_status_order IN('4','5') AND order_list_status='2' AND product_notify='2' ORDER BY order_list_code ASC";
    // }else{
    //     $status="AND order_list_status_order='".$_POST["idStatus"]."' AND order_list_status='2' AND product_notify='2' ORDER BY order_list_code ASC";
    // }
    ...
    $rowCount=$db->fn_fetch_rowcount("view_orders WHERE order_list_branch_fk='" . @$_SESSION["user_branch"] . "' $status ");
    if($rowCount>0){
        $query=array();
...
if(isset($_GET["countLabel"])){
    ...
    count(case when order_list_status_order='4' AND order_list_status!='1' then 1 end) as total4,
    count(case when order_list_status_order='5' AND order_list_status!='1' then 1 end) as total5","res_orders_list 
